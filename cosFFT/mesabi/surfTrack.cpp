...
// FFTW plan
	...
	fftw_plan planPsi, iPlanPsi, planN, iPlanPsiGradx, iPlanPsiGrady, iPlanPsiGradz,
			  ...
			  iPlanDtPsi, iPlanCurvH, iPlanPsiDxx, iPlanPsiDyy, iPlanPsiDxy;
...
// Parameters, variables and arrays
	...
	double dt = 0.0001; //0.001 for nw = 16
	double dtd2 = dt/2.0;	
...
// Runtime defined vectors
	...
	std::vector<double> curvH(size*2*alloc_local);
	std::vector<double> psiDxx(size*2*alloc_local);
	std::vector<double> psiDyy(size*2*alloc_local);
	std::vector<double> psiDxy(size*2*alloc_local);
	//std::vector<double> Ux(size*2*alloc_local);
	//std::vector<double> Uy(size*2*alloc_local);
	//std::vector<double> Uz(size*2*alloc_local);
	...
	std::vector<double> curvH_local (2*alloc_local);
	std::vector<double> psiDxx_local (2*alloc_local);
	std::vector<double> psiDyy_local (2*alloc_local);
	std::vector<double> psiDxy_local (2*alloc_local);
	...
	std::vector< std::complex<double> > curvHq_local (alloc_local);
	std::vector< std::complex<double> > psiqDxx_local (alloc_local);
	std::vector< std::complex<double> > psiqDyy_local (alloc_local);
	std::vector< std::complex<double> > psiqDxy_local (alloc_local);
...
// Crank-Nicolson + Adams-Bashforth scheme constants in FS
	...
	for ( i_local = 0; i_local < local_n0; i_local++ )
	{
		...
		for ( k = 0; k < Nz/2+1; k++ )
		{
			...
			opSH[index] = alpha*pow(mq2[index]-q02,2);
...
// Create backward FFTW plan
	...
	iPlanCurvH = fftw_mpi_plan_dft_c2r_3d(Nx,Ny,Nz,
	             		reinterpret_cast<fftw_complex*>(curvHq_local.data()),
				        curvH_local.data(),
				        MPI::COMM_WORLD,FFTW_MEASURE);

	iPlanPsiDxx = fftw_mpi_plan_dft_c2r_3d(Nx,Ny,Nz,
	             		reinterpret_cast<fftw_complex*>(psiqDxx_local.data()),
				        psiDxx_local.data(),
				        MPI::COMM_WORLD,FFTW_MEASURE);

	iPlanPsiDyy = fftw_mpi_plan_dft_c2r_3d(Nx,Ny,Nz,
	             		reinterpret_cast<fftw_complex*>(psiqDyy_local.data()),
				        psiDyy_local.data(),
				        MPI::COMM_WORLD,FFTW_MEASURE);

	iPlanPsiDxy = fftw_mpi_plan_dft_c2r_3d(Nx,Ny,Nz,
	             		reinterpret_cast<fftw_complex*>(psiqDxy_local.data()),
				        psiDxy_local.data(),
				        MPI::COMM_WORLD,FFTW_MEASURE);
...
// B. Initial condition - Read profile data
	...
	if ( rank == 0 )
	{
		...
		{
			index = (i*Ny + j) * (2*(Nz/2+1)) + k;	
			dpsiz = (psiTemp[index+1]-psiTemp[index-1])/(2*dz);
		  //psi[index] = psiTemp[index]+dpsiz*0.02*(cos(Qi*i*dx)+cos(Qi*j*dy));
		    psi[index] = psiTemp[index]+dpsiz*0.1*(cos(Qi*i*dx));

		//	psi[index] = psiTemp[index]+dpsiz*0.2*cos(Qi*i*dx)*sin(Qi*j*dy);
...
// Compute New psi in Fourier Space and transform back
		...
		for ( k = 0; k < Nz/2+1; k++ )
		{
			...
			dTpsiq_local[index] = scale*(aLin[index]*psiq_local[index]+Nq_local[index]);
			curvHq_local[index] = iMag*Vqx[i_local]*psiqGradx_local[index]
							    + iMag*Vqy[j]*psiqGrady_local[index];
			
			psiqDxx_local[index] = iMag*Vqx[i_local]*psiqGradx_local[index];
			psiqDyy_local[index] = iMag*Vqy[j]*psiqGrady_local[index];
			psiqDxy_local[index] = iMag*Vqy[j]*psiqGradx_local[index];
	
		}}}

		fftw_execute(iPlanPsi);
		fftw_execute(iPlanDtPsi);
		fftw_execute(iPlanCurvH);
...
// Track surface (inside count L1)
		...
		MPI::COMM_WORLD.Gather(curvH_local.data(),2*alloc_local,MPI::DOUBLE,
    	 	    			   curvH.data(),2*alloc_local, MPI::DOUBLE,0);	
		
		MPI::COMM_WORLD.Gather(psiDxx_local.data(),2*alloc_local,MPI::DOUBLE,
    	 	    			   psiDxx.data(),2*alloc_local, MPI::DOUBLE,0);	
		
		MPI::COMM_WORLD.Gather(psiDyy_local.data(),2*alloc_local,MPI::DOUBLE,
    	 	    			   psiDyy.data(),2*alloc_local, MPI::DOUBLE,0);	
	
		MPI::COMM_WORLD.Gather(psiDxy_local.data(),2*alloc_local,MPI::DOUBLE,
    	 	    			   psiDxy.data(),2*alloc_local, MPI::DOUBLE,0);	
	
// Temp add for velocity

	//	MPI::COMM_WORLD.Gather(Ux_local.data(),2*alloc_local,MPI::DOUBLE,
    //	 	    			   Ux.data(),2*alloc_local, MPI::DOUBLE,0);	
		
	//	MPI::COMM_WORLD.Gather(Uy_local.data(),2*alloc_local,MPI::DOUBLE,
    //	 	    			   Uy.data(),2*alloc_local, MPI::DOUBLE,0);	
	
	//	MPI::COMM_WORLD.Gather(Uz_local.data(),2*alloc_local,MPI::DOUBLE,
    //	 	    			   Uz.data(),2*alloc_local, MPI::DOUBLE,0);	
	...
	//		std::ofstream Ux_output("velUx.dat");
	//		std::ofstream Uy_output("velUy.dat");
	//		std::ofstream Uz_output("velUz.dat");

			assert(surf_output.is_open());
			assert(velS_output.is_open());
			assert(curvH_output.is_open());
			assert(curvK_output.is_open());
	//		assert(Ux_output.is_open());
	//		assert(Uy_output.is_open());
	//		assert(Uz_output.is_open());
			...
			{
				index = (i*Ny + j) * (2*(Nz/2+1)) + k;
				if ( psi[index] > 0.1 & track == 0 )
				{
					track = 1;
		    	}
				if ( psi[index] < 0 & track == 1 ) //std::abs()
				{
				if( std::abs(psi[index]) > std::abs(psi[index-1]) ) // >
				{
					index = index - 1;
				}
					surf_output << k << "\n ";
					...
					velSurf = dTpsi[index]/gradVal;
					velS_output << velSurf << "\n ";
					curvH_output << curvH[index]/gradVal << "\n ";
					curvK = psiDxx[index]*psiDyy[index]
						  - psiDxy[index]*psiDxy[index];
					curvK_output << curvK/(gradVal*gradVal) << "\n ";
	//				Ux_output << Ux[index] << "\n ";	
	//				Uy_output << Uy[index] << "\n ";	
	//				Uz_output << Uz[index] << "\n ";	

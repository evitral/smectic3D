 ...
 *     K: Not sure if the right method of K *              
 *                                          *
 *     It seems to be an old version        *
 *     but I kept it for legacy             *
 *					    *
 *     Note: this code obtains all          *
 *     surface info from the phase field    *
 *     parameter                            *
 *                                          *
 *     Last mod: 10/30/2017                 *
...
/* Indices and mpi related numbers */
	...
	int i, j, k, index, index1, index2, t, i_local, rank, size;
...
/* B. Initial condition - Read profile data */
	...
	/** Open file and obtain IC for global psi **/
    	...
    	std::ifstream psidata("psiHole.dat");
...
/* For the parallel planes aligned w/ z (surfTrack) 
	...
	for ( j = 0; j < Ny; j++ ) {
		...
		{
			index = (i_local*Ny + j) * Nz + k;
			if ( psi_local[index] > 0.05 & track == 0 )
			{
				track = 1;
	    	}
			if ( psi_local[index] > 0.0 & track == 1 ) //std::abs() SIGN CHAN
			{
				...
				curvH_local[index2] = (psiDxx_local[index]+psiDyy_local[index])/
									   gradVal;

				curvK_local[index2] = (psiDxx_local[index]*psiDyy_local[index]
				 	                - psiDxy_local[index]*psiDxy_local[index]) /
							          (gradVal*gradVal);
	...
	for ( i_local = 0; i_local < local_n0; i_local++ ) {
		...
		for ( k = Nz-1; k > -1; k-- ) 
		{
			...
			if ( psi_local[index] < 0.0 & track == 1 ) //std::abs(...) > 0.7
			{
				...
				/* Old way of computing curvatures

				nxx = (psiDxx_local[index] - psiGradx_local[index]*normGradx_local[index]/gradVal)/gradVal;
				//nxy = (psiDxy_local[index] - psiGradx_local[index]*normGrady_local[index]/gradVal)/gradVal;
				//nxz = (psiDxz_local[index] - psiGradx_local[index]*normGradz_local[index]/gradVal)/gradVal;

				//nyx = (psiDxy_local[index] - psiGrady_local[index]*normGradx_local[index]/gradVal)/gradVal;
				nyy = (psiDyy_local[index] - psiGrady_local[index]*normGrady_local[index]/gradVal)/gradVal;
				//nyz = (psiDyz_local[index] - psiGrady_local[index]*normGradz_local[index]/gradVal)/gradVal;

				//nzx = (psiDxz_local[index] - psiGradz_local[index]*normGradx_local[index]/gradVal)/gradVal;
				//nzy = (psiDyz_local[index] - psiGradz_local[index]*normGrady_local[index]/gradVal)/gradVal;
				nzz = (psiDzz_local[index] - psiGradz_local[index]*normGradz_local[index]/gradVal)/gradVal;

				curvH_local[index2] = nxx + nyy + nzz;
				
				curvK_local[index2] = ((psiGradz_local[index]*(psiDxx_local[index]*psiGradz_local[index]-2*psiGradx_local[index]*psiDxz_local[index])
									    + pow(psiGradx_local[index],2)*psiDzz_local[index])
									   *(psiGradz_local[index]*(psiDyy_local[index]*psiGradz_local[index]-2*psiGrady_local[index]*psiDyz_local[index])
										 + pow(psiGrady_local[index],2)*psiDzz_local[index])
									   - pow(psiGradz_local[index]*(-psiGradx_local[index]*psiDyz_local[index]+psiDxy_local[index]*psiGradz_local[index]
																 -psiDxz_local[index]*psiGrady_local[index])
											 + psiGradx_local[index]*psiGrady_local[index]*psiDzz_local[index],2))
					/(pow(psiGradz_local[index],2)*gradVal*gradVal);

 ...
 *                                          *
 *     Same thing as pf3d, but with some    *
 *     more interesting defects :)          *
 *                                          *
 ********************************************/
...
// Parameters, variables and arrays
	...
	double dt = 0.0005; //0.001 for nw = 16
	double dtd2 = dt/2.0;	
	...
	std::complex<double> mq2, aLin, dotSqVq;

// Constants and variables for defects (Nx = Ny = Nz)
...
// Crank-Nicolson + Adams-Bashforth scheme constants in FS
	...
	for ( i_local = 0; i_local < local_n0; i_local++ )
	{
		...
		for ( k = 0; k < Nz/2+1; k++ )
		{
			...
			aLin = epsilon - opSH[index];
			CM1[index] = 1.0/(rho*eta*mq2);
			CM2x[index] = Vqx[i_local]/mq2; 
			CM2y[index] = Vqy[j]/mq2;
			CM2z[index] = Vqz[k]/mq2;
...
//				psi_local[index] = Amp*cos(q0*k*dz);
				...
				if (ds < mid)
				{
					...
					{
						psi_local[index] = Amp*cos(q0*dz*sqrt(pow((bE/aE)*(ds-mid),2)+zs*zs));
					}
				}
				else
				{
	...
	for ( k = 0; k < Nz; k++ )
	{
		...
		if ( k == mid)
		{
			Nr_local[index] = beta*pow(psi_local[index],3) 
							...
							+ 2*wMid*(psi_local[index-1]-2*psi_local[index]
							+ psi_local[index+1])/(dz*dz);
		}
		else
		{
			Nr_local[index] = beta*pow(psi_local[index],3) 
